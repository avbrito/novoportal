...
const InfoProducao = () => {
  ...
  return (
    <div className="p-4 bg-white dark:bg-verde-dark">
      <div className="min-h-screen bg-white dark:bg-verde-dark">
        <div className="max-w-[808px] w-full mx-auto p-8 bg-white rounded-lg">
          ...
          <div className="bg-gray-card border-blue-lightest border border-solid p-8 rounded-lg left-0 w-96">
            ...
            <div className="mx-auto grid gap-6 mt-6">
                  ...
                  (item: { label: string; value: string }, index: number) => (
                    <div key={index} className="flex flex-col">
                      <div className="flex justify-between text-gray-700 mt-2">
                        <span className="text-blue-dark font-sans text-sm">{item.label}</span>
                        <span className="text-blue-black text-sm font-sansBold">{item.value}</span>
                      </div>
                      {index <
              ...
              <hr className="my-2 border-blue-lightest"/>
              <div>
                <h3 className="text-red-erro text-sm font-semibold">
                  Débitos
                </h3>
                <hr className="my-2 border-blue-lightest" />
                {dataFaturamento[selectedPeriod].debitos.map(
                  (item: { label: string; value: string }, index: number) => (
                    <div key={index} className="flex flex-col">
                      <div className="flex justify-between text-gray-700 mt-2">
                        <span className="text-blue-dark font-sans font-normal text-sm">{item.label}</span>
                        <span className="text-blue-black text-sm font-sansBold">{item.value}</span>
                      </div>
                      {index <
